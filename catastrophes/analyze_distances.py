...
# figure with 1x2 subplot structure
...
axarr[0,0].set_ylabel( r'$x$', fontsize=16 )
...
for i,s in enumerate( steps ):
    avg = avgs[s]
    nx = np.arange( 0.1*tmax, 0.9*tmax, s )[:-1]
    ax2.plot( nx, avg, lw=2, label=r'window size ($w$): '+str(s) )
    if plot_lr:
        lr1 = linreg1[s]
        lr2 = linreg2[s]
        if i == 0:
...
ax2.set_ylabel( r'Wasserstein 2-norm $d(P_i^w, P_{i+1}^w)$', fontsize=label_size )
